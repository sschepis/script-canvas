...
// started running.
//
import { tObject, tPoint, tLine, tMesh, tMaterial, tPlane, tSphere, tCube, tLight, tCamera, tScene, tRenderer, } from "./utility-classes";
    ...
    drawOverlay() {
        ...
        if (this.stack.length >= this._fps)
            for (var i = 0; i < this.stack.length; i++) {
                path.lineTo(this.x + 10 + i * (100 / this.stack.length), 9 + this.y + (100 / this.stack.length) - this.stack[i] / 2);
            }
